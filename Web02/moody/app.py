...
admin = User.objects(username="admin", password="admin")
...
def recommend(happy):
    ...
    all_film_admin = Film.objects(happy=happy, user_id=admin_id)
    film_admin = sample(all_film_admin,4)
    ...
    all_film_user = Film.objects(happy=happy, user_id__ne=admin_id)
    film_user = choice(all_film_user)
...
def add():
    if  request.method == "GET":
        ...
        new_book = Book(happy = form["happy"],
                        ...
                        user_id = session["user_id"])
        new_book.save()
        new_film = Film(happy = form["happy"],
                        name = ChuanHoa(form["name"]),
...
def admin_user():
    if "user_id" in session:
        ...
        all_film_user = Film.objects(happy=happy, user_id=session["user_id"])
...
def delete_film(film_id):
    film = Film.objects.with_id(film_id)
    if film is None:
        return "not found"
    else:
        film.delete()
...
def update_film(film_id):
    ...
    if film is None:
        ...
        if  request.method == "GET":
            ...
            form = request.form
            film.update(set__happy = form["happy"],
                        set__name = ChuanHoa(form["name"]),
                        set__director = form["singer"],
                        set__link = form["link"],
                        set__link_img = form["link_img"],
                        )
            film.reload()
